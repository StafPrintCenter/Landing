  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-card shadow-lg"
      >
        {/* Header */}
        ...
        {!submitted ? (
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-4">
                  ...
                  <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-2.5 text-sm">
                    ...
                    {/* Affichage conditionnel du message s'il existe */}
                    {watchNotes && (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <button type="button" onClick={() => { onClose(); setStep(1); setSubmitted(false); }} className="mt-6 rounded-full border border-border px-6 py-2.5 text-sm font-semibold hover:bg-muted">
              Fermer la fenêtre
            </button>
          </motion.div>
        )}
