...
function CheckOfferPage() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error("Aucune candidature ne correspond à l'email et la clé de suivi renseignés.");
        }
        if (response.status === 422) {
          const errData = await response.json();
          throw new Error(errData.message || "Les données transmises sont invalides.");
        }
  ...
  return (
    <div className="container max-w-2xl mx-auto px-4 py-12 md:py-16">
      ...
      {/* État : Erreur */}
      {!isLoading && error && (
        <CheckErrorState message={error} onRetry={handleRetry} />
      )}

      {/* État : Succès (Résultat) */}
      {!isLoading && !error && application && (
        <ApplicationResult application={application} onReset={handleReset} />
      )}

      {/* État : Formulaire de recherche initial */}
      {!isLoading && !error && !application && (
        <CheckForm
          email={email}
